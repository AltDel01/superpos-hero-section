      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="overflow-x-auto">
          <Table className="w-full bg-white rounded-lg shadow-lg">
            ...
            <TableBody>
              {comparisons.map((item, index) => <TableRow key={item.feature} className={index === comparisons.length - 1 ? 'rounded-b-lg' : ''}>
                  <TableCell className={`font-medium ${index === comparisons.length - 1 ? 'rounded-bl-lg' : ''}`}>
                    {item.feature}
                  </TableCell>
                  <TableCell className="text-center">{item.superpos}</TableCell>
                  <TableCell className="text-center">{item.custom}</TableCell>
                  <TableCell className={`text-center ${index === comparisons.length - 1 ? 'rounded-br-lg' : ''}`}>
                    {item.other}
                  </TableCell>
                </TableRow>)}
            </TableBody>
